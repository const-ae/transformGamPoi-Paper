...
res %>%
  ...
  left_join(trans_families) %>%
  left_join(dataset_summaries %>% dplyr::select(-c(row_means, row_vars))) %>%
  mutate(log_n_cells = log(n_cells), log_cputime_sec = log(cputime_sec)) %>%
  nest_by(transformation) %>% 
  mutate(fit = list(lm(log_cputime_sec ~ log_n_cells, data = data))) %>%
  summarize(broom::tidy(fit)) %>%
  ggplot(aes(x = transformation, y = estimate)) +
    geom_point() +
