...
trans_labels <- c("acosh" = r"($\textrm{acosh}(2\alpha y/s+1)$)", "logp_alpha" = r"($\log(y/s+1/(4\alpha))$)", "logp_cpm" = r"($\log(\textrm{CPM}+1)$)",
                  "logp1" = r"($\log(y/s+1)$)", "logp1_hvg" = r"($\log(y/s+1)\rightarrow\textrm{HVG}$)", "logp1_hvg_zscore" = r"($\log(y/s+1)\rightarrow\textrm{HVG}\rightarrow\textrm{Z}$)", 
                  "logp1_zscore" = r"($\log(y/s+1)\rightarrow\textrm{Z}$)", "logp1_size_normed" = r"($\log(y/s+1)/u$)",
                  "pearson" = "Pearson (no clip)", "pearson_analytic" = "Analytic Pearson", "pearson_clip" = "Pearson",
                  "rand_quantile" = "Random Quantile", "sctransform" = "sctransform",
...
trans_families_labels <- c(delta_method = "Delta Method", glm_residual = "GLM Residuals", latent_expr = "Latent Expr.")
