...
Y <- size_factor_transformation_res$raw
  ...
  group_map(function(dat, key){
    ...
    ggplot(dat, aes(x = gene_mean, y = gene_var))+ 
      ...
      theme(strip.text = element_blank(), strip.background = element_blank(), plot.title = element_blank(),
            ggh4x.axis.ticks.length.minor = rel(0.5), axis.text = element_blank()) 
  })
...
mouse_lung_pl[[2]] <- cowplot::ggdraw(mouse_lung_pl[[2]]) +
  cowplot::draw_label(x = 0.5, y = 0.6, label = "Type II pneumocytes", size = font_size_small) +
  cowplot::draw_label(x = 0.43, y = 0.5, label = "Other cells", size = font_size_small) +
  cowplot::draw_line(x = c(0.65, 0.83), y = c(0.55, 0.2), size = 0.25) +
  cowplot::draw_line(x = c(0.43, 0.41), y = c(0.45, 0.2), size = 0.2) 
mouse_lung_pl
```
...
plot_assemble(
  ...
  annotate_text("(A) Residual structure due to sequencing depth of a RNA solution encapsulated in droplets", 
  ...
  annotate_text(paste0("(B) Mean-variance relation of transformated values for ", 
                       prettyNum(n_genes, big.mark = ","), " genes of the GSE130931 dataset"),
  ...
  annotate_text(paste0("(C) Distribution of transformed values for one gene (Sftpc) with a bimodal expression pattern in lung epithelium"), 
