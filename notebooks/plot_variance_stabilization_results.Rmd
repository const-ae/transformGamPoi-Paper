...
overdisp <- glmGamPoi::glm_gp(sce, overdispersion = "global", overdispersion_shrinkage = FALSE, on_disk = FALSE)$overdispersions[1]
```
...
mean_var_plots <- mean_var_df %>%
  ...
  mutate(family = factor(family, levels = c("raw", "delta_method", "glm_residual", "latent_expr", "count_model"))) %>%
  ...
  group_map(function(dat, key){
    ...
    ggplot(dat, aes(x = gene_mean, y = gene_var))+ 
      ...
      labs(x = "Gene Mean (log-scale)", y = "Variance", title = if(cat ==  "raw") "Raw" else trans_families_labels[as.character(cat)]) +
      theme(strip.text = element_text(size = font_size_small),
            strip.background = element_blank()) 
  })

cowplot::plot_grid(plotlist = mean_var_plots, nrow = 4, align = "v", axis = "l", rel_heights = c(1.2, 2, 2, 1.2))
save_plot("../output/suppl-mean_var_relation.pdf", last_plot(), width = 150, height = 180, units = "mm", latex_support = TRUE)
```
