...
make_comparison_plot <- function(cond_1, cond_2, label_cond_1 = cond_1, label_cond_2 = cond_2, add_dataset_label = FALSE, drop_na_level = FALSE){
  ...
  ggplot(tmp, aes(x = mean_diff, y = benchmark)) +
      geom_vline(xintercept = 0, size = 0.3, linetype = "dotted", color = "grey") +
      geom_pointrange(aes(xmin = mean_ci_low, xmax = mean_ci_high, group = as.factor(knn), color = direction), size = 0.5, fatten = 1.2) +
      annotate("segment", xend = extreme_val, x = extreme_val * 0.1, y = arrow_pos, yend = arrow_pos, arrow = arrow(length = unit(font_size_tiny / 2, "pt"))) +
      annotate("segment", xend = -extreme_val, x = -extreme_val * 0.1, y = arrow_pos, yend = arrow_pos, arrow = arrow(length = unit(font_size_tiny / 2, "pt"))) +
      annotate("text", x = -extreme_val * 0.55, y = arrow_pos + 0.2, label = label_cond_1, vjust = 0, size = font_size_tiny / .pt) +
      annotate("text", x = -extreme_val * 0.55, y = arrow_pos - 0.1, label =" better", vjust = 1, size = font_size_tiny / .pt) +
      annotate("text", x =  extreme_val * 0.55, y = arrow_pos + 0.2, label = label_cond_2, vjust = 0, size = font_size_tiny / .pt) +
      annotate("text", x =  extreme_val * 0.55, y = arrow_pos - 0.1, label = " better", vjust = 1, size = font_size_tiny / .pt) +
      (if(add_dataset_label) geom_text(aes(x = mean_ci_high, label = benchmark), hjust = -0.1, size = font_size_tiny / .pt, color = "#444444")
       else NULL) +
      ...
      scale_y_discrete(expand = expansion(add = c(0, 1.5)), drop = drop_na_level) +
...
post_proc_comps_pl <- cowplot::plot_grid(plotlist = map2(post_proc_comps, c(TRUE, rep(FALSE, 3)), 
                                                         ~ make_comparison_plot(.x[[1]], .x[[2]], .x[[3]], .x[[4]], add_dataset_label = .y)),
                                         ncol = 4, align = "vh")

cross_comps_pl <- cowplot::plot_grid(plotlist = map2(cross_proc_comps, c(TRUE, rep(FALSE, 3),TRUE, rep(FALSE, 3)),
                                                     ~ make_comparison_plot(.x[[1]], .x[[2]], .x[[3]], .x[[4]], add_dataset_label = .y)),
                                     nrow = 2, ncol = 4, align = "vh")
```
...
knn_per_depth_pl <- full_knn_overlaps %>%
  ...
  ggplot(aes(x = seq_depth, y = overlap)) +
    ...
    labs(y = "KNN Overlap", x = "UMIs / cell",
         title = "(D) More UMIs/cell, better KNN overlap") +
...
plot_assemble(
  ...
  list(plot = cross_comps_pl, x = 0, y = 6, width = 112, height = 35),
  annotate_text("(B) Non-zero overdispersion improves\nresults for Pearson residuals and acosh", fontsize = font_size, x = 120, y = 2, vjust = 1, fontface = "bold"),
  list(plot = alpha_comps_pl, x = 120, y = 8, width = 60, height = 30),
  annotate_text("(C) Post-processing does not consistently improve VST results", fontsize = font_size, x = 0, y = 42, vjust = 1, fontface = "bold"),
  list(plot = post_proc_comps_pl, x = 0, y = 46, width = 112, height = 17.5),
  list(plot = knn_per_depth_pl, x = 120, y = 38, width = 60, height = 29),
  width = 180, height = 67, units = "mm", show_grid_lines = FALSE,
