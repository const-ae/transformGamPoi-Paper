...
annot_df <- bind_rows(tibble(row_means = 5e3, row_vars = 5e3, label = r"($\mathbb{V}\textrm{ar}=\mu$)",
                             dataset_label = c("GSE130931", "Dyngen", "HEK (ss3)"), 
                             benchmark = c("Consistency", "Simulation", "Downsampling"), hjust = 0, vjust = 0.5, angle = 45),
                      tibble(row_means = sqrt(4e3), row_vars = 4e3, label = r"($\mathbb{V}\textrm{ar}=\mu+1\mu^2$)",
                             dataset_label = c("GSE130931", "Dyngen", "HEK (ss3)"), 
                             benchmark = c("Consistency", "Simulation", "Downsampling"), hjust = 0, vjust = 0.4, angle = atan(2) / pi * 180),
                      tibble(row_means = sqrt(5e3 / 0.01), row_vars = 5e3, label = r"($\mathbb{V}\textrm{ar}=\mu+0.01mu^2$)",
                             dataset_label = c("GSE130931", "Dyngen", "HEK (ss3)"), 
                             benchmark = c("Consistency", "Simulation", "Downsampling"), hjust = 0, vjust = 0.4, angle = atan(2) / pi * 180),
  ...
  group_map(function(dat, key){
    ...
    ggplot(dat, aes(x = tsne[,1], y = tsne[,2])) +
      ...
      theme(axis.ticks = element_blank(), axis.text = element_blank(), axis.line = element_blank(),
            ...
            strip.text = element_text(size = font_size_tiny), strip.background = element_blank()) +
