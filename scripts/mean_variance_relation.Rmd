...
if(! file.exists("../data/hek-smartseq3.Rds")){
  download.file("https://www.ebi.ac.uk/arrayexpress/files/E-MTAB-8735/E-MTAB-8735.processed.3.zip", "../data/smart_seq3_data.zip")
  unzip("../data/smart_seq3_data.zip", files = "Smartseq3.HEK.cleanup.UMIcounts.txt", exdir = "../data")
  raw_data <- read.delim("../data/Smartseq3.HEK.cleanup.UMIcounts.txt") %>%
    as.matrix() %>%
    as("dgCMatrix")
  saveRDS(SingleCellExperiment(list(counts = raw_data), rowData = DataFrame(gene_id = rownames(raw_data))), "../data/hek-smartseq3.Rds")
}
...
se <- readRDS("../data/hek-smartseq3.Rds")
...
tibble(mu, var, cell_cycle_gene = rownames(se_red) %in% human_cell_cycle_genes) %>%
  ...
  ggplot(aes(x = mu, y = var)) +
    ...
    scale_y_log10(breaks = c(0.001, 0.1, 10, 1000, 1e5), limits = c(1e-3, 1e6),
                  ...
                  name = expression(Variance)) +
    scale_color_manual(values = c("TRUE" = "#87172f", "FALSE" = "black")) +
    ggtitle("HEK Cells (Smart-seq3)", subtitle = "") +
    theme(plot.subtitle = ggtext::element_markdown())
...
p_bio <- cowplot::plot_grid(p1, p3, p4, p2, nrow = 1, align = "h")
