...
method_labeller <- c("logp1" = "log(x+1)", "logp_alpha" = "log(x+1/(4*alpha))", "acosh" = "acosh(2*alpha*x+1)",
                   "pearson" = "Pearson~Residulas", "rand_quantile" = "Rand.~Quantile~Residuals",
                   "sanity" = "Sanity", counts = "Raw~Counts")

limits <- tibble(counts = 16000, logp1 = 10.5, logp_alpha = 10.5, acosh = 10.5,
                 pearson = 10.5, rand_quantile = 10.5, sanity = 4) %>%
...
res <- tibble(gene = rownames(subset_sce),
       ...
       sanity = matrix(rnorm(n = nrow(subset_sce) * ncol(subset_sce), mean = sane_res$mean, sd = sane_res$sd), nrow = nrow(subset_sce), ncol = ncol(subset_sce))) %>%
       # sanity = sane_res$mean) %>%   
  ...
  mutate(transformation = factor(transformation, levels  = c("pearson", "rand_quantile", "sanity", "logp1", "logp_alpha", "acosh", "counts"))) %>%
...
cowplot::save_plot("../output/variance_after_transformation.pdf", res, nrow = 3, ncol = 3, base_height = 2, base_asp = 2)
```
